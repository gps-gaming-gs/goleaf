...
import (
	...
	"github.com/gps-gaming-gs/goleaf/rpcx/generator"
...
var (
	// VarStringCCController describes the Controller Name.
	VarStringCCController string
	// VarStringCCProxy describes the Proxy Name.
	VarStringCCProxy string
	// VarStringCocosProjectName describes the cocos project.
	VarStringCocosProjectName string
...
func RPCNew(_ *cobra.Command, args []string) error {
	...
	cocosProjectName := VarStringCocosProjectName
	if len(remote) > 0 {
	...
	ctx.CocosProjectName = cocosProjectName
...
// CocosTemplate is the entry for generate cocos template
func CocosTemplate(latest bool) error {
	if !latest {
		console.Warning("deprecated: goctl ccController template -ccController is deprecated and will be removed in the future, use goctl cocos -controller instead")
	}
	ctrlName := VarStringCCController
	proxyName := VarStringCCProxy
	home := VarStringHome
	remote := VarStringRemote
	branch := VarStringBranch
	if len(remote) > 0 {
	...
	if len(ctrlName) > 0 {
		return generator.CCControllerTmpl(ctrlName)
	}

	if len(proxyName) > 0 {
		return generator.CCProxyTmpl(proxyName)
	}

	return errors.New("goleaf cocos -h 查看可用指令")
}
