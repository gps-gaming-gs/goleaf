...
func ZRPC(_ *cobra.Command, args []string) error {
	...
	if len(grpcOutList) == 0 {
		grpcOutList = append(grpcOutList, "./server/msg")
		protocArgs = append(protocArgs, "--go_out ./server/msg")
		//return errInvalidGrpcOutput
	}
	if len(goOutList) == 0 {
		goOutList = append(goOutList, "./server/msg")
		protocArgs = append(protocArgs, "--go-grpc_out ./server/msg")
