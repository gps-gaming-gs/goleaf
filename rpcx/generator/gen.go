...
type ZRpcContext struct {
	...
	Multiple bool
	// Cocos is the flag to create cocos project
	CocosProjectName string
}
...
func (g *Generator) Generate(zctx *ZRpcContext) error {
	...
	err = g.GenSkeleton(dirCtx, proto, g.cfg)
	if err != nil {
		return err
	}

	err = g.GenConf(dirCtx, proto, g.cfg)
	if err != nil {
		return err
	}

	err = g.GenGame(dirCtx, proto, g.cfg)
	if err != nil {
		return err
	}

	err = g.GenGameData(dirCtx, proto, g.cfg)
	if err != nil {
		return err
	}

	err = g.GenGate(dirCtx, proto, g.cfg)
	if err != nil {
		return err
	}

	err = g.GenMsg(dirCtx, proto, g.cfg)
	if err != nil {
		return err
	}

	err = g.GenLogin(dirCtx, proto, g.cfg)
	if err != nil {
		return err
	}

	err = g.GenPb(dirCtx, proto, zctx)
	if err != nil {
		return err
	}
	...
	// Cocos
	if len(zctx.CocosProjectName) > 0 {
		cocosDirCtx, err := mkdircocos(projectCtx, zctx)
		err = g.GenCocos(cocosDirCtx, proto, g.cfg)
		if err != nil {
			return err
		}
	}
