...
export abstract class BezierCurveMoveBase extends cc.Component {
    /* --------------- 属性 --------------- */
    /** 当前下标变更事件 */
    @property({
    ...
    /** 移动缓动 */
    protected _moveTween: cc.Tween<any> | null = null;
    /** 循环移动状态 */
     ...
     * @returns
     */
    move(indexN_: number, scrollConfig_?: BezierCurveMoveBase_.MoveConfig): void {
        if (this._moveB && !this._loopRunB) {
            return;
        }
        this._moveB = true;
        this._moveConfig = new BezierCurveMoveBase_.MoveConfig(scrollConfig_);
    ...
    /** 获取运动速度 */
    getSpeed(indexN_: number, scrollConfig_?: BezierCurveMoveBase_.GetSpeedConfig): number {
        scrollConfig_ = new BezierCurveMoveBase_.GetSpeedConfig(scrollConfig_);
        /** 目标距离 */
